...
public class ReceiptsRetrievalVerifier {
    ...
    public void requestReceiptsFromPeers(List<AionBlock> blocks, String originNodeDisplayId,
        int originNodeHashId) {
    ...
    public void validateAgainstBlockchain(AionTxInfo receivedInfo) {
        ...
        if (!Arrays.equals(
            ...
            throw new IllegalArgumentException(
                String.format("Can't compare AionTxInfo of different hashes (%s / %s)",
                    receivedInfo.getReceipt().getTransaction().getTransactionHash(),
                    blockchainInfo.getReceipt().getTransaction().getTransactionHash()));
        }
    ...
    /**
     * Validate that alternate tx db and canonical tx db returns same data when given a tx hash
     * key.
     */
    public void validateDatabases(AionTxInfo receivedInfo, TransactionStore alternateTxStore) {
        ...
        List<AionTxInfo> canonicalDbInfos = AionRepositoryImpl.inst().getTransactionStore()
            .get(hash);

        String result = altDbInfos.equals(canonicalDbInfos) ? "SAME" : "DIFFERENT";
        LOG.info(
            String.format("ReceiptsRetrievalVerifier result=%s altDbInfos=%s canonicalDbInfos=%s",
                result,
                altDbInfos,
                canonicalDbInfos));
    }
